...
results_df <- read_tsv("/inwosu/imbalanced_learn/Data/results/accuracy_file.txt", col_names = F) 
...
png("/inwosu/Meta_Analysis/Data/cross_validation_results/imb_learn.png", width = 18, height = 10, units = "in", res = 300)
results_df %>%
  ggplot(aes(x = factor(Resampling_strategy), y = `roc_auc_score`, fill = factor(Model))) +
  theme(text = element_text(size = 15)) +
  geom_jitter(color = "black", size = 0.3, alpha = 0.4) +
